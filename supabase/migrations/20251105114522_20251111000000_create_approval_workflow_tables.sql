/*
  ...
  1. **dim_approval_rules** - Defines approval requirements based on PR value thresholds
  2. **fact_pr_approval_history** - Tracks all approval actions and decisions
  3. **vw_pr_list** - Consolidated view of PRs with current approval status
...
SELECT
  ...
  CASE
    ...
    WHEN ph.pr_status = 'Pending Approval' THEN
      (SELECT COALESCE(elem->>'approver_role', 'Unknown')
...
VALUES
  ('Small Purchase (< IDR 10M)', 0, 10000000, '[{"role": "Department Manager", "level": 1, "reason": "Standard approval for purchases under IDR 10M"}]'::jsonb, 2),
  ('Medium Purchase (IDR 10M - 50M)', 10000000, 50000000, '[{"role": "Department Manager", "level": 1, "reason": "Initial review and recommendation"},{"role": "Finance Manager", "level": 2, "reason": "Budget verification for purchases IDR 10M-50M"}]'::jsonb, 3),
  ('Large Purchase (IDR 50M - 100M)', 50000000, 100000000, '[{"role": "Department Manager", "level": 1, "reason": "Initial review and justification"},{"role": "Finance Manager", "level": 2, "reason": "Budget and financial compliance check"},{"role": "Director", "level": 3, "reason": "Executive approval required for purchases over IDR 50M"}]'::jsonb, 5),
  ('Strategic Purchase (> IDR 100M)', 100000000, 999999999999, '[{"role": "Department Manager", "level": 1, "reason": "Technical and operational justification"},{"role": "Finance Manager", "level": 2, "reason": "Financial due diligence"},{"role": "Director", "level": 3, "reason": "Executive review"},{"role": "Board of Directors", "level": 4, "reason": "Strategic investment approval for purchases over IDR 100M"}]'::jsonb, 7)
