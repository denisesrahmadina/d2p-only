...
import { X, Upload, FileSpreadsheet, AlertTriangle, CheckCircle, Loader, File, FileText, TestTube, Users } from 'lucide-react';
...
interface ImportUserGuideModalProps {
  isOpen: boolean;
  onClose: () => void;
  onImport: (userGuideData: UserGuideData) => void;
}

const ImportUserGuideModal: React.FC<ImportUserGuideModalProps> = ({
  isOpen,
  onClose,
  onImport
}) => {
  const [formData, setFormData] = useState<UserGuideData>({
    name: '',
    description: '',
    type: 'user guide',
    category: '',
  ...
  const handleInputChange = (field: keyof UserGuideData, value: string | string[]) => {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      let guideType = 'User Guide';
      
      if (fileName.includes('finance') || fileName.includes('financial')) {
        guideType = 'Finance Operations User Guide';
      } else if (fileName.includes('hr') || fileName.includes('human')) {
        guideType = 'HR Services User Guide';
      } else if (fileName.includes('it') || fileName.includes('support')) {
        guideType = 'IT Support User Guide';
      } else if (fileName.includes('procurement') || fileName.includes('purchase')) {
        guideType = 'Procurement User Guide';
      } else if (fileName.includes('analytics') || fileName.includes('data')) {
        guideType = 'Analytics User Guide';
      } else if (fileName.includes('compliance') || fileName.includes('audit')) {
        guideType = 'Compliance User Guide';
      }
      ...
      const mockUrl = `https://ssc-knowledge.s3.amazonaws.com/user-guides/${file.name.replace(/\s+/g, '-').toLowerCase()}`;
      const mockPreviewUrl = `https://ssc-knowledge.s3.amazonaws.com/user-guides/preview/${file.name.replace(/\s+/g, '-').toLowerCase().replace(/\.[^/.]+$/, '.html')}`;
      ...
      setFormData(prev => ({
        ...
        category: guideType.includes('Finance') ? 'Finance Operations' :
                 guideType.includes('HR') ? 'HR Services' :
                 guideType.includes('IT') ? 'IT Support Services' :
                 guideType.includes('Procurement') ? 'Procurement Services' :
                 guideType.includes('Analytics') ? 'Data Analytics & Reporting' :
                 guideType.includes('Compliance') ? 'Compliance & Risk Management' :
                 'General Operations'
      }));
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.name.trim()) errors.push('User guide name is required');
    ...
    if (!selectedFile) errors.push('User guide document file is required');
    ...
    if (fileAnalysis && !fileAnalysis.hasValidStructure) {
      errors.push('Document file does not have valid user guide structure');
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const userGuideData: UserGuideData = {
        ...formData,
      ...
      console.error('Error importing user guide:', error);
      setValidationErrors(['Failed to import user guide. Please try again.']);
    } finally {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] flex flex-col">
        {/* Modal Header */}
        <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-purple-50 to-blue-50 dark:from-purple-900/20 dark:to-blue-900/20 rounded-t-lg">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h2 className="text-lg font-semibold text-gray-900 dark:text-white">
                Import User Guide Document
              </h2>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Upload user guide and training materials for SSC operations
              </p>
            </div>
          </div>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* User Guide Document File Upload */}
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                User Guide Document File *
              </label>
              ...
              {!selectedFile ? (
                <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-6 text-center hover:border-purple-400 dark:hover:border-purple-500 transition-colors">
                  ...
                  />
                  <label
                    htmlFor="userguide-file-upload"
                    className="cursor-pointer flex flex-col items-center space-y-3"
                  >
                    <div className="w-12 h-12 bg-purple-100 dark:bg-purple-900/50 rounded-lg flex items-center justify-center">
                      {isFileUploading ? (
                        <Loader className="h-6 w-6 text-purple-600 dark:text-purple-400 animate-spin" />
                      ) : (
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-900 dark:text-white">
                        {isFileUploading ? 'Analyzing user guide document...' : 'Upload User Guide Document'}
                      </p>
                  ...
                  <div className="bg-purple-50 dark:bg-purple-900/30 rounded-lg p-4 border border-purple-200 dark:border-purple-800">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <p className="text-sm font-medium text-purple-900 dark:text-purple-300">{selectedFile.name}</p>
                      ...
                      <button
                        onClick={clearFile}
                        disabled={isProcessing}
                        className="p-2 rounded-md text-purple-600 dark:text-purple-400 hover:bg-purple-100 dark:hover:bg-purple-800 transition-colors disabled:opacity-50"
                    ...
                    <div className="bg-green-50 dark:bg-green-900/30 rounded-lg p-4 border border-green-200 dark:border-green-800">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        ...
                        <div>
                          ...
                          <span className="ml-2 text-green-900 dark:text-green-100">{fileAnalysis.guideType}</span>
                        </div>
                      </div>
                    </div>
                  )}
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                Upload your user guide document. The system will analyze and categorize it automatically.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., Finance Operations User Guide"
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Describe the user guide purpose, scope, and key processes covered..."
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g., user-guide, training, navigation, tutorial (comma-separated)"
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                disabled={isProcessing}
              />
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., Analytics Director"
                  className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                  disabled={isProcessing}
                  required
                />
        ...
        <div className="border-t border-gray-200 dark:border-gray-700 p-6 bg-white dark:bg-gray-800 rounded-b-lg">
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-600 dark:text-gray-400">
              <p>The user guide document will be analyzed and made available in the knowledge base.</p>
            ...
            <div className="flex items-center space-x-3">
              ...
              <button
                ...
                className="flex text-sm items-center space-x-2 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2"
              >
                {isProcessing ? (
                  <>
...
export default ImportUserGuideModal;