...
const UserGuideTab: React.FC<UserGuideTabProps> = ({ selectedOrganization }) => {
  const [userGuideDocuments, setUserGuideDocuments] = useState<KnowledgeDocument[]>([]);
  ...
  // Load user guide documents data
  React.useEffect(() => {
    const loadUserGuideDocuments = async () => {
      try {
        setLoading(true);
        const documentsData = await KnowledgeService.getKnowledgeDocumentsByType('user guide');
        setUserGuideDocuments(documentsData);
      } catch (error) {
        console.error('Error loading user guide documents:', error);
        setUserGuideDocuments([]);
      } finally {
        setLoading(false);
      }
    };

    loadUserGuideDocuments();
  }, []);
  ...
  const filteredAndSortedDocuments = React.useMemo(() => {
    let filtered = userGuideDocuments;
    ...
    return filtered;
  }, [userGuideDocuments, searchTerm, categoryFilter, statusFilter, uploaderFilter, sortField, sortDirection]);
  ...
  const uniqueCategories = [...new Set(userGuideDocuments.map(doc => doc.category))].sort();
  const uniqueUploaders = [...new Set(userGuideDocuments.map(doc => doc.uploadedBy))].sort();
    ...
    return (
      <div className="space-y-6">
        <div className="text-center py-8">
          ...
          <p className="text-gray-600 dark:text-gray-400">Loading user guide documents...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow-sm border border-gray-200 dark:border-gray-800 p-4">
        <h3 className="text-sm font-semibold text-gray-900 dark:text-white mb-3 flex items-center space-x-2">
          ...
          <span>Search & Filter User Guide Documents</span>
        </h3>
        ...
        {(searchTerm || categoryFilter || statusFilter || uploaderFilter) && (
          <div className="mt-3 flex items-center justify-between">
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Showing {filteredAndSortedDocuments.length} of {userGuideDocuments.length} user guides
            </p>
            <button
              onClick={() => {
                setSearchTerm('');
        ...
        <div className="p-4 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm text-gray-500 dark:text-gray-400">
              Page {currentPage} of {totalPages} • {filteredAndSortedDocuments.length} user guides
            </div>
          </div>
        </div>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center space-x-2">
                        <button
                          ...
                          title="Download user guide document"
                        >
          ...
          <div className="px-4 py-3 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50">
            <div className="flex items-center justify-between">
              <div className="text-sm text-gray-700 dark:text-gray-300">
                Showing {startIndex + 1} to {Math.min(endIndex, filteredAndSortedDocuments.length)} of {filteredAndSortedDocuments.length} user guides
              </div>
              <div className="flex items-center space-x-2">
                <button
        ...
        {paginatedDocuments.length === 0 && (
          <div className="text-center py-8">
            <p className="text-gray-500 dark:text-gray-400">
              {userGuideDocuments.length === 0 ? 'No user guide documents found' : 'No user guides match your current filters'}
            </p>
          </div>
        )}
      </div>
