...
import { FileText, Calendar, CheckCircle, ChevronRight, Search, Filter, Upload } from 'lucide-react';
...
interface DocumentSubmissionListProps {
  onSelectEvent: (event: SourcingEvent) => void;
}

const DocumentSubmissionList: React.FC<DocumentSubmissionListProps> = ({ onSelectEvent }) => {
  ...
  const loadEvents = async () => {
    ...
    try {
      ...
      const { data: sectionsData, error: sectionsError } = await supabase
        ...
        .select('sourcing_event_id');

      if (sectionsError) throw sectionsError;

      const counts: { [key: string]: number } = {};
      sectionsData?.forEach(section => {
        counts[section.sourcing_event_id] = (counts[section.sourcing_event_id] || 0) + 1;
      });
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600 dark:text-gray-400">
              Configure submission requirements for tenders
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Available Events', value: events.length, icon: FileText },
          { label: 'Configured', value: Object.keys(sectionCounts).length, icon: CheckCircle },
          { label: 'Pending Setup', value: events.length - Object.keys(sectionCounts).length, icon: Calendar }
        ].map((stat, idx) => (
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center space-x-2 mb-2">
                    ...
                    {sectionCounts[event.sourcing_event_id] > 0 && (
                      <span className="px-2 py-1 text-xs font-medium rounded-full bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-400">
                        {sectionCounts[event.sourcing_event_id]} sections
                      </span>
                    )}
                  </div>
                  <p className="text-xs text-gray-600 dark:text-gray-400 mb-3">
                    ID: {event.sourcing_event_id}
                  </p>
